...
func gain_strength_xp(amount: float) -> void:
	strength_xp += amount
	print("[STATS-FORZA] XP: +", amount, " | Totale: ", strength_xp, "/", strength_xp_required)
	...
	if strength_xp >= strength_xp_required:
		...
		print("[LEVEL-UP] FORZA: Livello ", strength_level)
		
		# --- NUOVO: Controllo Bivio Abilità ---
		if strength_level in [5, 10, 15, 20]:
			strength_skill_point_earned.emit(strength_level)
			print("[SISTEMA] Ottenuto punto abilità FORZA al livello ", strength_level, "!")
		# --------------------------------------
...
func gain_stealth_xp(amount: float) -> void:
	stealth_xp += amount
	print("[STATS-STEALTH] XP: +", amount, " | Totale: ", stealth_xp, "/", stealth_xp_required)
	...
	if stealth_xp >= stealth_xp_required:
		...
		print("[LEVEL-UP] STEALTH: Livello ", stealth_level)
		
		# --- NUOVO: Controllo Bivio Abilità ---
		if stealth_level in [5, 10, 15, 20]:
			stealth_skill_point_earned.emit(stealth_level)
			print("[SISTEMA] Ottenuto punto abilità STEALTH al livello ", stealth_level, "!")
		# --------------------------------------
